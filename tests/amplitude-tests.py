...
source_dirs = [30]
amplitudes = [0.01, 0.05, 0.1, 0.25, 0.5]
snr = 20  # dB

## Source and noise setup
sources = [("pure", "1000Hz"),("sample", "glass-10khz")]
source_noise_pairs: List[Tuple[int, str, str, str, float, float, SoundSource]]
source_noise_pairs = []
for src in sources:
    type, name = src
    for amp in amplitudes:
        ...
        if type == "pure":
            ...
            amplitude = max_amp * amp_factor
        expected_signal = amplitude * (math.e ** (-decay_rate * source_dist))
        noise_amplitude = math.sqrt(expected_signal**2/(10**(snr/20)))
        print(f'{name} {snr} src amp {amplitude} expected signal {expected_signal} noise amp {noise_amplitude}')
        source_noise_pairs.append((snr, "white", name, type, amplitude, amp_factor, WhiteNoiseSoundSource(sample_rate, noise_amplitude)))
...
for sound_noise, dir in itertools.product(source_noise_pairs, source_dirs):
    for i in range(4):
        snr, noise_type, sound_name, src_type, src_amplitude, amp_factor, noise_source = sound_noise # unpack tuple
        ...
        save_wav_samples(mic1.samples, sample_rate, sound_name, test_name, str(round(src_amplitude,2)), noise_type, "combined")
        save_wav_samples(mic1.original_samples, sample_rate, sound_name, test_name, str(round(src_amplitude,2)), noise_type, "original")
        save_wav_samples(mic1.noise_samples, sample_rate, sound_name, test_name, str(round(src_amplitude,2)), noise_type, "noise")
