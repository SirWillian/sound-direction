...
csvwriter.writerow(["mic_shape", "shape_side", "sound_amplitude", "sound_name", "sound_dist", "detection_window", "sound_dir", "detected_dir", "detection_error", "sim_time", "detection_time"])
...
for src, detection_window in itertools.product(sources, detection_windows):
    ...
    print(''); print(f'{repr(sound_source)} {source_dist}m {source_dir}deg {detection_window}s window')
    ...
    csvwriter.writerow(["equi", mic_tri_side, src_amplitude, sound_name, source_dist, detection_window, source_dir, direction, abs(source_dir-direction), sim_real_time, detection_time])
    save_wav_samples(mic1.samples, sample_rate, sound_name, test_name, str(detection_window))
